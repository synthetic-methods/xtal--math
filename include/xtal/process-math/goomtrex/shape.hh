...
struct shape<M_ism, 0>
{
	...
	public:
		...
		noexcept -> XTAL_ALL_(u)
		{
			...
			XTAL_IF0
			XTAL_0IF (N_ord <  0) {
				return term_f(sinh(u), cosh(u) - U_op::alpha_1, v);
			}
			XTAL_0IF (3 == M_ism) {
...
struct shape<M_ism, -1>
{
	...
	public:
		...
		noexcept -> decltype(auto)
		{
			...
			XTAL_0IF (2 == N_ord) {
				auto const w0 = term_f(1, U_co(1, 60), w), w1 =   square_f(w0)*U_co(3, 20);
				auto const u0 = term_f(1, U_co(1, 30), w), u1 = v*square_f(u0)*U_co(1,  2);
				U y{1};
				y  = term_f(y, w, w1);
				y  = term_f(y, u, u1);
				y *= w0;
			...
			XTAL_0IF_(else) {
				...
				for (int i{0}; i < N_ord; ++i) {
					q *= U_3*term_f<1, 2>(1, u_M*q);
				}
				q *= U_1/U_N;
				auto const p = root_f<2>(term_f<1, 2>(1, u*q)) - U_1;
