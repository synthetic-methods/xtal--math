...
TAG_("shape")
{
	...
	EST_("shape_t<3>::function<~0, 1>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			//\
			o *= shape_t<3, -0>::template function<~0, 1>(_op::mantissa_f(mt19937_f));
			o *= exp(_op::mantissa_f(mt19937_f));
		}
		return o;

	};
	EST_("shape_t<3>::function< 7, 1>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			o *= shape_t<3, -0>::template function< 7, 1>(_op::mantissa_f(mt19937_f));
		}
		return o;

	};
	EST_("shape_t<3>::function< 3, 1>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			o *= shape_t<3, -0>::template function< 3, 1>(_op::mantissa_f(mt19937_f));
		}
		return o;

	};
	EST_("shape_t<3>::function< 2, 1>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			o *= shape_t<3, -0>::template function< 2, 1>(_op::mantissa_f(mt19937_f));
		}
		return o;

	};
	EST_("shape_t<3>::function< 1, 1>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			o *= shape_t<3, -0>::template function< 1, 1>(_op::mantissa_f(mt19937_f));
		}
		return o;
	...
	EST_("shape_t<3>::function< 7, 0>")
	{
		T_alpha o{1};

		for (int i{0}; i < 0x100; ++i) {
			o *= shape_t<3, -0>::template function< 7, 0>(_op::mantissa_f(mt19937_f));
		}
		return o;
