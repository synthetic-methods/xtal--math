	...
	public:
		...
		noexcept -> decltype(auto)
		{
			...
			if constexpr (N_lim < 0) {
				...
				XTAL_0IF (M_ism ==  1) {return root_f<2>(term_f<-1>(_1, _2, w))/term_f<-1>(_1, _4, XTAL_REF_(w));}
				XTAL_0IF (M_ism == -1) {auto const m = term_f<  >(_1, _8, XTAL_REF_(w)); return _1/root_f<2>(_op::haplo_1*(root_f<2>(m) + m));}
				XTAL_0IF (M_ism == -2) {auto const m = term_f<-1>(_1, _4, XTAL_REF_(w)); return _1/root_f<2>(_op::haplo_1*(root_f<2>(m) + m));}
			}
		}
