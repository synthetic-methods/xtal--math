...
TAG_("series")
{
	...
	EST_("series reinitialization")
	{
		series_t<T_aphex[4]> foo{};
	...
	TRY_("series reinitialization")
	{
		using W_aphex = series_t<T_aphex[2]>;
	...
	{
		auto constexpr N_size = 2;
		using W = series_t<T_alpha[N_size]>;

		auto foo = W{2.0, 0.5};
		auto bar = process::math::roots_f<2>((T_alpha) 2);
		auto baz = bar*bar;
	...
	{
		using U0 = T_sigma;
		using U1 = series_t<U0[1]>;
		using U2 = series_t<U0[2]>;
		using U3 = series_t<U0[3]>;
		using U4 = series_t<U0[4]>;
	...
	TRY_("referencing")
	{
		...
		using simplex_val = series_t<float   [N]>;
		using simplex_ref = series_t<float(&)[N]>;
