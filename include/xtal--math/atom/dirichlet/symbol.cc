...
TAG_("symbol")
{
	...
	TRY_("11th characterization (complex)")
	{
		...
		_detail::apply_to<[] (auto &&z) XTAL_0FN_(return) (arg(XTAL_REF_(z))*K/_fit::patio_1)>(w);

		_detail::apply_to<[] XTAL_0FN_(alias) (_fit::template trim_f<16>)>(m);
		_detail::apply_to<[] XTAL_0FN_(alias) (_fit::template trim_f<16>)>(w);
