...
\note    Uses the tangent-formula from `sine_t` to ensure accuracy at the limit.
*/
template <int M_ism=0, int M_car=0>
struct tangent;

template <int M_ism=0, int M_car=0>
using   tangent_t = process::confined_t<tangent<M_ism, M_car>>;

template <int M_ism=0, int M_car=0, auto ...Ns>
XTAL_DEF_(let)
tangent_f = [] XTAL_1FN_(call) (tangent_t<M_ism, M_car>::template method_f<Ns...>);
...
struct tangent<M_ism, -0>
{
	...
	public:
		...
		noexcept -> decltype(auto)
		{
			auto const o_sgn = aspect_f<signed>(o);
