...
struct root
{
	...
	protected:
		...
		noexcept -> objective_t<XTAL_ALL_(z)>
		{
			...
			auto constexpr Z_cut = M_cut == 0? 0: Z_fit::minilon_f(M_cut);
			...
			XTAL_0IF (1 <= M_cut and real_variable_q<Z>) {
				...
				return Z_fit::alpha_1/(XTAL_REF_(z) + z_cut);
			}
			XTAL_0IF (M_cut <= 0 and real_variable_q<Z>) {
				return Z_fit::alpha_1/XTAL_REF_(z);
			}
			XTAL_0IF (complex_variable_q<Z> and real_variable_q<typename Z::value_type>) {
			// Approximating `-fcx-fortran-rules`...
				using namespace bond::math;
				auto const z_re =  z.real(); auto i_re = bit_exponent_f(z_re); i_re &= ~bit_sign_f(i_re);
				auto const z_im = -z.imag(); auto i_im = bit_exponent_f(z_im); i_im &= ~bit_sign_f(i_im);
				auto const  _i  =  Z_fit::haplo_f(i_re + i_im >> 1);
				auto const  _2  =  square_f(_i)/square_f(z_re*_i, z_im*_i, Z_cut);
				return complexion_f(XTAL_MOV_(z_re)*_2, XTAL_MOV_(z_im)*_2);
			}
			XTAL_0IF_(else) {
				return Z_fit::alpha_1/XTAL_REF_(z);// TODO: Handle generic `0 < M_cut`!
			}
		}
