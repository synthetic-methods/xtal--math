	...
	protected:
		...
		noexcept -> objective_t<XTAL_ALL_(z)>
		{
			...
			// Emulating `-fcx-fortran-rules`...
				auto const z_re =  z.real(); auto i_dn = Z_exp&bit_exchange_f(z_re);
				auto const z_im = -z.imag(); auto i_up = Z_exp&bit_exchange_f(z_im);
				(void) bond::math::bit_swap_f<+1>(i_dn, i_up);
				auto const z_i  =  bit_exchange_f(Z_exp - i_up);
