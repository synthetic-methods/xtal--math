...
TAG_("component")
{
	...
	TRY_("magnum edition")
	{
		...
		TRUE_(check_f<-1>(component_t<unsigned>::edit_f(alpha), alpha_mgn));
		TRUE_(check_f<-1>(component_t<unsigned>::edit_f(aphex), aphex_mgn));
		...
		TRUE_(same_q<float, decltype(component_f<unsigned>(+1.F))>);
		TRUE_(same_q<float, decltype(component_f<unsigned>(-1.F))>);
	...
	TRY_("signum edition")
	{
		...
		TRUE_(check_f<-1>(component_t<signed>::edit_f(alpha), alpha_sgn));
		TRUE_(check_f<-1>(component_t<signed>::edit_f(aphex), aphex_sgn));
	...
	TRY_("assigned_f( 1)")
	{
		TRUE_( 1. == component_f<signed>(T_sigma{ 1}));
		TRUE_( 1. == component_f<signed>(T_delta{ 1}));
		TRUE_( 1. == component_f<signed>(T_alpha{ 1}));
	//	TRUE_( 1. == component_f<signed>(true));

	};
	TRY_("assigned_f( 0)")
	{
		TRUE_(-1. == component_f<signed>(T_sigma{ 0}));
		TRUE_(-1. == component_f<signed>(T_delta{ 0}));
	//	TRUE_(-1. == component_f<signed>(T_alpha{-0}));
	//	TRUE_(-1. == component_f<signed>(false));
