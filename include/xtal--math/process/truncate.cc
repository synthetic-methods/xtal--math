...
TAG_("truncate")
{
	...
	//	TRUE_(truncate_f<range>(0) == 0);
	//	TRUE_(truncate_f<range>(2) == 1);
	...
	{
		TRUE_(truncate_f< 2>(N_min) > N_min);
		TRUE_(truncate_f<-2>(N_max) < N_max);
#ifndef __FINITE_MATH_ONLY__
		TRUE_(truncate_f<-2>(N_inf) < N_inf);
#endif
	};
	TRY_("threshold evaluation")
	{
		...
		TRUE_(truncate_f<+half>(0.25) == 0.5);
		TRUE_(truncate_f<-half>(0.75) == 0.5);

		TRUE_(truncate_f<[] XTAL_1FN_(to) ( U_fit::upsilon_f(2)*U_fit::haplo_1)>(0.5) > 0.5);
		TRUE_(truncate_f<[] XTAL_1FN_(to) ( U_fit::dnsilon_f(2)*U_fit::haplo_1)>(0.5) < 0.5);

		TRUE_(truncate_f<[] XTAL_1FN_(to) (+U_fit::minilon_f(2))>(N_min) > N_min);
		TRUE_(truncate_f<[] XTAL_1FN_(to) (-U_fit::maxilon_f(2))>(N_max) < N_max);
#ifndef __FINITE_MATH_ONLY__
		TRUE_(truncate_f<[] XTAL_1FN_(to) (-U_fit::maxilon_f(2))>(N_inf) < N_inf);
#endif

		TRUE_(truncate_f<[] XTAL_1FN_(to) ( U_fit::upsilon_f(2)*U_fit::minilon_f(2))>(N_min) > N_min);
		TRUE_(truncate_f<[] XTAL_1FN_(to) ( U_fit::dnsilon_f(2)*U_fit::maxilon_f(2))>(N_max) < N_max);
#ifndef __FINITE_MATH_ONLY__
		TRUE_(truncate_f<[] XTAL_1FN_(to) ( U_fit::dnsilon_f(2)*U_fit::maxilon_f(2))>(N_inf) < N_inf);
#endif
