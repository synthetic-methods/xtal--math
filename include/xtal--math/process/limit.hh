...
struct limit<M_app>
{
	...
	static auto constexpr M_abs = component_f<unsigned>(M_val);
	static auto constexpr M_sgn = component_f<  signed>(M_val);
	static auto constexpr M_dir = (int) component_f<signed>(nearing_f(M_abs)*(M_sgn));
		...
		requires un_n<complex_variable_q<decltype(w_)>>
		{
			...
			//	auto &[w0, w1] = w_;
				...
				XTAL_0IF (complex_variable_q<W0> and atom::couple_q<W1>) {
					return W_{W0{one, zero}, edit_f<Ns...>(w1)};
				}
				XTAL_0IF (atom::couple_q<W0> and complex_variable_q<W1>) {
					return W_{edit_f<Ns...>(w1), W1{one, zero}};
				}
				//\
			...
			//	TODO: Accommodate returning materialized `atom::block` from `span`s...
				return [&]<auto ...I> (bond::seek_t<I...>)
					XTAL_0FN_(to) (W(edit_f<Ns...>(get<I>(w_))...))
