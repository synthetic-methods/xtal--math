...
template <bond::tagged_as_q<dent>  T > XTAL_TYP_(new) in_dent<T> {using type = typename T::data_type;};
template <class T                    > XTAL_TYP_(set) in_dent_t = typename in_dent<based_t<T>>::type;
template <             typename ...Ts> XTAL_TYP_(ask) in_dent_q = bond::tagged_as_p<dent, Ts...>;
