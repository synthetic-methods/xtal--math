...
noexcept -> auto
{
	...
	if constexpr (U_fit::use_IEC) {
		...
		x  -= U_fit::    unit. mark;
		return bit_axis_f<N_dir>(_xtd::bit_cast<U_delta>(XTAL_MOV_(x)));
	}
	else {
		return bit_axis_f<N_dir>(   static_cast<U_delta>(_std::ilogb(XTAL_MOV_(u))));
	}
}
template <int N_dir=0>
XTAL_DEF_(return,inline,let)
...
XTAL_DEF_(return,inline,let)
bit_clasp_f(integral_q auto x)
noexcept -> auto
{
	x -= N_mask; x &= bit_sign_f(x);
	x += N_mask;
	return x;
}
template <  signed int N_depth>
XTAL_DEF_(return,inline,let)
bit_clamp_f(integral_q auto const x)
noexcept -> auto
{
	int constexpr N_width = 1 << N_depth;
...
////////////////////////////////////////////////////////////////////////////////
/*!
\returns The fractional part of `x` as a full-width `delta_type`.
*/
